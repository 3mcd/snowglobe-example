...
export default class CapsuleBufferGeometry extends BufferGeometry {
  constructor(
    ...
    function calculateVertexCount() {
      let count =
        (radialSegments + 1) * (heightSegments + 1 + capsBottomSegments + capsTopSegments)
      return count
    }

    function calculateIndexCount() {
      let count =
        radialSegments * (heightSegments + capsBottomSegments + capsTopSegments) * 2 * 3
      return count
    }
    ...
    function generateTorso() {
      ...
      const vl = radiusTop * alpha + cone_length + radiusBottom * (Math.PI / 2 - alpha)
      ...
      for (y = 1; y <= heightSegments; y++) {
        ...
        // calculate the radius of the current row
        let radius =
          sinAlpha * ((y * (radiusBottom - radiusTop)) / heightSegments + radiusTop)
